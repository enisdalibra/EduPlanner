...
| --- | --- | --- |
| Offline IndexedDB storage | Complete | Dexie schema version 8 with academic-period and class-enrollment history. |
...
| Class management | Complete | Classes belong to academic periods, support read-only archives, and can promote selected students into one or multiple target classes. Permanent deletion removes class-owned records and enrollments without deleting student profiles. |
...
| Student management | Complete | A single student profile can join multiple classes. Excel import previews and links an existing NIS instead of duplicating the profile; direct student deletion still removes all student-owned records. |
